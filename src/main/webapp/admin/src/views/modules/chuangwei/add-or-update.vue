...
export default {
  ...
  methods: {
    ...
    fangjianhaoChange () {
      this.$http({
        url: `follow/fangjianziliao/fangjianhao?columnValue=`+ this.ruleForm.fangjianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.loufangmingcheng){
                this.ruleForm.loufangmingcheng = data.data.loufangmingcheng
              }
        } else {
          this.$message.error(data.msg);
        }
