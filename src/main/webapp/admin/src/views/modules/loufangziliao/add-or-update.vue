<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="装修程度" prop="zhuangxiuchengdu">
          <el-input v-model="ruleForm.zhuangxiuchengdu" 
              placeholder="装修程度" clearable  :readonly="ro.zhuangxiuchengdu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="装修程度" prop="zhuangxiuchengdu">
              <el-input v-model="ruleForm.zhuangxiuchengdu" 
                placeholder="装修程度" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="建立时间" prop="jianlishijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.jianlishijian" 
                type="datetime"
                :readonly="ro.jianlishijian"
                placeholder="建立时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jianlishijian" label="建立时间" prop="jianlishijian">
              <el-input v-model="ruleForm.jianlishijian" 
                placeholder="建立时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.loufangtupian" label="楼房图片" prop="loufangtupian">
          <file-upload
          tip="点击上传楼房图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.loufangtupian?ruleForm.loufangtupian:''"
          @change="loufangtupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.loufangtupian" label="楼房图片" prop="loufangtupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.loufangtupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhandimianji = true;
          ...
          if(o=='zhuangxiuchengdu'){
            this.ruleForm.zhuangxiuchengdu = obj[o];
	    this.ro.zhuangxiuchengdu = true;
            continue;
          }
          if(o=='xiangqing'){
	    ...
	    this.ro.jianlishijian = true;
          ...
          if(o=='loufangtupian'){
            this.ruleForm.loufangtupian = obj[o];
	    this.ro.loufangtupian = true;
            continue;
          }
        }
      }
	...
	if(this.ruleForm.loufangtupian!=null) {
		this.ruleForm.loufangtupian = this.ruleForm.loufangtupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `loufangziliao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.loufangziliaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    loufangtupianUploadChange(fileUrls) {
	this.ruleForm.loufangtupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
