<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="药品名称" prop="yaopinmingcheng">
          <el-select :disabled="ro.yaopinmingcheng" v-model="ruleForm.yaopinmingcheng" placeholder="请选择药品名称">
            <el-option
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yongyaoqingkuang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yongyaoqingkuangCrossAddOrUpdateFlag = false;
						 this.parent.search();
