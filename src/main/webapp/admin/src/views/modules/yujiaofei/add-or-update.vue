<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="入住编号" prop="ruzhubianhao">
          <el-select :disabled="ro.ruzhubianhao" @change="ruzhubianhaoChange" v-model="ruleForm.ruzhubianhao" placeholder="请选择入住编号">
            <el-option
                v-for="(item,index) in ruzhubianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.pingzheng" label="凭证" prop="pingzheng">
          <file-upload
          tip="点击上传凭证"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.pingzheng" label="凭证" prop="pingzheng">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.pingzheng.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="预交费" prop="yujiaofei">
          <el-input v-model="ruleForm.yujiaofei" 
              placeholder="预交费" clearable  :readonly="ro.yujiaofei"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="预交费" prop="yujiaofei">
              <el-input v-model="ruleForm.yujiaofei" 
                placeholder="预交费" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="说明" prop="shuoming">
                <el-input
...
export default {
  data() {
	...
	ispay : false,
      ...
      rules: {
          ...
          yujiaofei: [
                { required: true, message: '预交费不能为空', trigger: 'blur' },
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.pingzheng = true;
          ...
          if(o=='yujiaofei'){
            this.ruleForm.yujiaofei = obj[o];
	    this.ro.yujiaofei = true;
            continue;
          }
          if(o=='shuoming'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/laorenruzhu/ruzhubianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.ruzhubianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    ruzhubianhaoChange () {
      this.$http({
        url: `follow/laorenruzhu/ruzhubianhao?columnValue=`+ this.ruleForm.ruzhubianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              ...
              if(data.data.jiashuzhanghao){
                this.ruleForm.jiashuzhanghao = data.data.jiashuzhanghao
              }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yujiaofei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yujiaofeiCrossAddOrUpdateFlag = false;
						 this.parent.search();
