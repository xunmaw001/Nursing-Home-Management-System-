<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="新房间号" prop="xinfangjianhao">
          <el-select :disabled="ro.xinfangjianhao" v-model="ruleForm.xinfangjianhao" placeholder="请选择新房间号">
            <el-option
                v-for="(item,index) in xinfangjianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="新房间号" prop="xinfangjianhao">
	      <el-input v-model="ruleForm.xinfangjianhao"
                placeholder="新房间号" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/chuangwei/fangjianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.xinfangjianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhuanfangdengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhuanfangdengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
